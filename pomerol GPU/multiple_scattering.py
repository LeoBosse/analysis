...
class MultipleScattering:
    ...
    def GetTotalContribution(self):
        ...
        # print(self.Flux_list, self.Stokes_I_list, self.Stokes_Q_list)
        V = 0
        Vc = 0
        Vs = 0
        ...
        for i_ray in range(self.nb_rays):
            I    = self.Stokes_I_list[i_ray]
            DoLP = self.Stokes_Q_list[i_ray]
    ...
    def SetStokesParameters(self):
        """Compute the Stokes paramters of each individual rays and stock them in a list"""
        self.Stokes_I_list = np.zeros(self.nb_rays)
        self.Stokes_Q_list = np.zeros(self.nb_rays)

        for i_ray in range(self.nb_rays):
            self.Stokes_I_list[i_ray], self.Stokes_Q_list[i_ray] = self.GetRayStokesParameters(i_ray)

            self.Stokes_I_list[i_ray] *= self.Flux_list[i_ray]

        # print("self.Stokes_I_list", self.Stokes_I_list)
        # print("self.Stokes_Q_list", self.Stokes_Q_list)

    def GetRayStokesParameters(self, ray_id):
