...
class Bottle:
    ...
    def GetSmoothLists(self):
        ...
        self.smooth_I0, self.smooth_DoLP, self.smooth_AoLP = np.zeros(
    ...
    def SetUnifyAngles(self):
        ...
        self.smooth_AoLP, smooth_graph_angle_shift = UnifyAngles(
            self.smooth_AoLP)
        ...
        if all_graph_angle_shift != self.graph_angle_shift:
            self.all_AoLP, all_graph_angle_shift = UnifyAngles(
                self.all_AoLP, manual_shift=self.graph_angle_shift)
        ...
        if smooth_graph_angle_shift != self.graph_angle_shift:
            self.smooth_AoLP, smooth_graph_angle_shift = UnifyAngles(
                self.smooth_AoLP, manual_shift=self.graph_angle_shift)
            self.smooth_AoLP_upper, tmp = UnifyAngles(
                self.smooth_AoLP_upper, manual_shift=self.graph_angle_shift)
            self.smooth_AoLP_lower, tmp = UnifyAngles(
                self.smooth_AoLP_lower, manual_shift=self.graph_angle_shift)
        ...
        if self.observation_type == "fixed" and self.azimut is not None and self.elevation is not None:
            ...
            if to_initiate:
                ...
                self.continue_rotation_times.append(
                    self.all_times[-1].total_seconds() / 60.)
    ...
    def GetOneTimePeriod(self, start_time, end_time, smooth_data=True, direct=False):
        ...
        if direct == True:
            I0 = self.all_I0[(start_time < self.all_times)
                             & (self.all_times < end_time)]
            DoLP = self.all_DoLP[(start_time < self.all_times)
                                 & (self.all_times < end_time)]
            AoLP = self.all_AoLP[(start_time < self.all_times)
                                 & (self.all_times < end_time)]
            nb_times = len(I0)
            return I0, DoLP, AoLP, nb_times
        else:
            V = self.all_V[(start_time < self.all_times)
                           & (self.all_times < end_time)]
            Vcos = self.all_Vcos[(start_time < self.all_times)
                                 & (self.all_times < end_time)]
            Vsin = self.all_Vsin[(start_time < self.all_times)
                                 & (self.all_times < end_time)]
            nb_times = len(V)
    ...
    def GetRotationFrequency(self):
        all_freq = [1. / (self.all_times[i].total_seconds()
                          - self.all_times[i - 1].total_seconds()) for i in range(1, self.nb_rot)]
    ...
    def GetInterpolation(self, time):
        ...
        interp_I0 = np.interp(time, all_times, self.smooth_I0)
        # , left=0, right=0, period=None)
        interp_DoLP = np.interp(time, all_times, self.smooth_DoLP)
        # , period=np.pi)
        interp_AoLP = np.interp(time, all_times, self.smooth_AoLP)
    ...
    def __add__(self, bottle_to_add):
        ...
        if len(bottle_to_add.rotations) > 0:  # If not read from a file
            ...
            self.smooth_Vcos = np.append(
                self.smooth_Vcos, bottle_to_add.smooth_Vcos)
            self.smooth_Vsin = np.append(
                self.smooth_Vsin, bottle_to_add.smooth_Vsin)

            self.nb_smooth_rot = len(self.smooth_V)

            # Calculate the smooth I0, DoLP and AoLP
            self.smooth_I0 = np.append(self.smooth_I0, bottle_to_add.smooth_I0)
            self.smooth_DoLP = np.append(
                self.smooth_DoLP, bottle_to_add.smooth_DoLP)
            self.smooth_AoLP = np.append(
                self.smooth_AoLP, bottle_to_add.smooth_AoLP)
...
class PTCUBottle(Bottle):
    ...
    def SaveTXT(self):
        ...
        data = pd.DataFrame(np.array([times, self.all_V, self.all_Vcos, self.all_Vsin, self.all_I0, self.all_DoLP, self.all_AoLP, self.smooth_V, self.smooth_Vcos, self.smooth_Vsin, self.smooth_I0,
                            self.smooth_DoLP, self.smooth_AoLP, self.std_I0, self.std_DoLP, self.std_AoLP, self.std_smooth_I0, self.std_smooth_DoLP, self.std_smooth_AoLP, self.all_SN, self.smooth_SN]).transpose())
        ...
        data.to_csv(self.data_file_name + "/" + self.saving_name
                    + '_results.txt', sep="\t", index=False)
    ...
    def SaveHDF5(self):
        ...
        data = [times, self.all_V, self.all_Vcos, self.all_Vsin, self.all_I0, self.all_DoLP, self.all_AoLP, self.smooth_V, self.smooth_Vcos, self.smooth_Vsin, self.smooth_I0,
                self.smooth_DoLP, self.smooth_AoLP, self.std_I0, self.std_DoLP, self.std_AoLP, self.std_smooth_I0, self.std_smooth_DoLP, self.std_smooth_AoLP, self.all_SN, self.smooth_SN]
    ...
    def LoadFromHDF5(self):
        ...
        self.all_times = np.array([dt.timedelta(milliseconds=t)
                                  for t in data["Time"]])
        self.nb_rot = len(self.all_times)

        self.SetJumps()

        self.smooth_V = np.array([d for d, t in zip(
            data["smooth_V"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_Vcos = np.array([d for d, t in zip(
            data["smooth_Vcos"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_Vsin = np.array([d for d, t in zip(
            data["smooth_Vsin"], self.all_times) if self.head_jump <= t < self.tail_jump])

        self.nb_smooth_rot = len(self.smooth_V)

        # Calculate the smooth I0, DoLP and AoLP
        self.smooth_I0 = np.array([d for d, t in zip(
            data["Smooth_I0"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_DoLP = np.array([d for d, t in zip(
            data["Smooth_DoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_AoLP = np.array([d for d, t in zip(
            data["Smooth_AoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])

        self.all_V = np.array([d for d, t in zip(
            data["PMAvg"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_Vcos = np.array([d for d, t in zip(
            data["PMCosAvg"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_Vsin = np.array([d for d, t in zip(
            data["PMSinAvg"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_I0 = np.array([d for d, t in zip(
            data["I0"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_DoLP = np.array([d for d, t in zip(
            data["DoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_AoLP = np.array([d for d, t in zip(
            data["AoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])

        self.std_I0 = np.array([d for d, t in zip(
            data["errI0"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.std_smooth_I0 = np.array([d for d, t in zip(
            data["errSI0"], self.all_times) if self.head_jump <= t < self.tail_jump])

        self.std_DoLP = np.array([d for d, t in zip(
            data["errDoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.std_smooth_DoLP = np.array([d for d, t in zip(
            data["errSDoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])

        self.std_AoLP = np.array([d for d, t in zip(
            data["errAoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.std_smooth_AoLP = np.array([d for d, t in zip(
            data["errSAoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])

        norm = self.head_jump
        self.all_times = np.array(
            [t - norm for t in self.all_times if self.head_jump <= t < self.tail_jump])
    ...
    def LoadFromTxt(self):
        ...
        self.smooth_V = np.array([d for d, t in zip(
            data["SV"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_Vcos = np.array([d for d, t in zip(
            data["SVcos"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_Vsin = np.array([d for d, t in zip(
            data["SVsin"], self.all_times) if self.head_jump <= t < self.tail_jump])

        self.nb_smooth_rot = len(self.smooth_V)

        # Calculate the smooth I0, DoLP and AoLP
        self.smooth_I0 = np.array([d for d, t in zip(
            data["SI0"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_DoLP = np.array([d for d, t in zip(
            data["SDoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_AoLP = np.array([d for d, t in zip(
            data["SAoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])

        self.all_V = np.array([d for d, t in zip(
            data["V"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_Vcos = np.array([d for d, t in zip(
            data["Vcos"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_Vsin = np.array([d for d, t in zip(
            data["Vsin"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_I0 = np.array([d for d, t in zip(
            data["I0"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.all_DoLP = np.array([d for d, t in zip(
            data["DoLP"], self.all_times) if self.head_jump <= t < self.tail_jump])
        ...
        self.all_SN = np.array([d for d, t in zip(
            data["SN"], self.all_times) if self.head_jump <= t < self.tail_jump])
        self.smooth_SN = np.array([d for d, t in zip(
            data["SSN"], self.all_times) if self.head_jump <= t < self.tail_jump])
...
class SPPBottle(Bottle):
    ...
    def SaveTXT(self):
        ...
        np.savetxt("/".join(self.data_file_name.split("/")[:-1]) + "/" + self.saving_name + '_results.txt',                    np.array([times, self.smooth_V, self.smooth_Vcos, self.smooth_Vsin,
                   self.smooth_I0, self.smooth_DoLP, self.smooth_AoLP]).transpose(), delimiter="\t", header="time (ms)\tV\tVcos\tVsin\tI0 (mV)\tDoLP (percent)\tAoLP (deg)")
