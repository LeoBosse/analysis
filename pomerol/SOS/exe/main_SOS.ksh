...
#       Value format : Encoding as real F7.3 (10 characters max).
#
...
#       Value format : Encoding as real F6.4 (10 characters max).
#
#    Keyword  : -SURF.Nadal.Beta  
#    ---------------------------   
#       Status : Associated to  -SURF.Type 6      
#       Value :  Parameter Beta of Nadal's BPDF model. 
#       Value format : Encoding as real F4.1 (10 characters max).
#
#
#-----------------------------------------------------------------
...
if [ "${SOS_Wa_status}" = "0" ]
then
    echo "ERROR ON PARAMETERS: Obligatory parameter missing :" 
...
#AerModel
if [ ${flagAOTref} -eq 1 ] && [ "${AER_Model_status}" = "0" ]
then
    echo "ERROR ON PARAMETERS: " 
...
### 3) Simulation of aerosols radiative properties       
	   ...
	   #     ====> Generating : File $AER_ResFile
		 ...
		 kmat1_waref=`head -1 ${AER_ResFile} | awk '{print $1}'`
		 ...
		 # --> Call SOS_AEROSOLS program : creating the file ${AER_ResFile}
	         ...
	         kmat1_wa=`head -1 ${AER_ResFile} | awk '{print $1}'`	     
