...
ctypedef np.float_t DTYPE_t_FLOAT
...
class MultipleScattering:
    ...
    def Propagate(self, cy.float alt, np.ndarray vec, np.ndarray scattering_plane, cy.int id_ray = 0, np.ndarray hist = np.zeros((0, 6)), cy.float segment_length = 0):

        cdef bint is_scattered = False
