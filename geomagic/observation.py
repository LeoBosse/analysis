...
class ObservationPoint:
    ...
    def GetPolaPlane(self, AoLP, mode='perp', coord = 'A'):
        ...
        A = np.array((0, np.sin(AoLP), np.cos(AoLP))).transpose() #in instrument coordinates (x*,y*,z*).
        los = np.array((1, 0, 0)).transpose() #in instrument coordinates (x*,y*,z*).

        norm = A #perp case
        if mode == 'para':
            norm = np.cross(A, los) #para case

        if coord.lower() in ['a', 'o']:
            R_IA = self.GetRotMatrixAI(transpose = True)
            norm = np.dot(R_IA, norm) #plane expressed in up-east-north coordinate
            # print("plane", plane)
        if coord.lower()  == 'o':
