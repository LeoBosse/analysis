...
class SkyMap:
	...
	def LoadSkyEmmisionsCube(self, Nb_a_pc, Nb_e_pc):
		...
		self.scattering_map 		= np.zeros(self.data_shape) # Intensity from (e, a) reaching us
		self.DoLP_map				= np.zeros(self.data_shape) # Polarized intensity from (e, a) reaching us
		self.total_scattering_map 	= np.zeros(self.data_shape) # DoLP of scattered light from (e,a)
